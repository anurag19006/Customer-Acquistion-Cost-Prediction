...
library(neuralnet)
library(MASS)
NeuralNetwork <- function(train.df,valid.df,key) {
  nn <- neuralnet(cost ~.,
                  data = train.df, hidden = c(3,1),
                  linear.output = TRUE)
  
  predictions <- compute(nn, valid.df[,1:109])$net.result
  test.features = subset(valid.df, select=-c(cost))
  test.target = subset(valid.df, select=cost)[,1]
...
NeuralNetwork(train.df,valid.df,'Decision Tree Regressor onOrdinal Encoded Data')
